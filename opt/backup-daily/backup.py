...
import sys, os,subprocess,re,time, socket, logging
...
def get_fs_type(path):
    try:
        ...
        logging.exception('Error getting filesystem type "%s"',e.message)
        return None
...
def snapshot_data(name):
    ...
    incremental = {
        ...
        "from_tag" : from_tag
    } if from_tag is not None and from_tag.get("tag") != "" else {}
...
def btrfs_list():
    try:
        ...
        logging.exception('Error listing BTRFS subvolumes "%s"', e.args[0])
        return []
...
def send_backup_using_bash(i,incremental=False):
    ...
    if incremental and snap.get('incremental',{}).get('target') is not None:
    ...
    if (incremental and from_tag_incremental is not None and from_tag_incremental != ''):
...
def backup(i):
    ...
    incremental_tag = i.get('incremental',{}).get('from_tag',{}).get('tag')
    if incremental_tag is not None and incremental_tag != '':
...
def mount_shares(block_device: str, mountpoint: str, options: str) -> bool:
    try:
        ...
        if mount.returncode == 0:
            ...
            logging.error(f'ERROR: Mount command failed with return code {mount.returncode}')
            return False
    except subprocess.CalledProcessError as e:
        logging.exception(f'Error while mounting backup {block_device} at mountpoint "{mountpoint}"')
...
def umount_shares(mountpoint):
    ...
    if umount.returncode == 0:
        ...
        logging.error(f'ERROR: Umount command failed with return code {umount.returncode}')
        return False
